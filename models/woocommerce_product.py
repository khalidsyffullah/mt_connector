...
class Product(models.Model):
    ...
    def create_product(self, p_item, wooc_instance):
        ...
        dict_p = {
            'wooc_id': p_item.get('id', ''),
            'woocomm_instance_id': wooc_instance.id,
            'name': p_item.get('name', ''),
            'company_id': wooc_instance.wooc_company_id.id,
            'type': 'product',
            'woocomm_product_status': p_item.get('status', ''),
            'woocomm_product_type': p_item.get('type', ''),
            'woocomm_regular_price': safe_float(p_item.get('regular_price')),
            'woocomm_sale_price': safe_float(p_item.get('sale_price')),
            'purchase_ok': True,
            'sale_ok': True,
            'is_exported': True,
            'is_woocomm_product': True,
            'is_product_active': p_item.get('status') == 'publish',
            'default_code': p_item.get('sku', ''),
        ...
        # Categories
        if p_item.get('categories'):
            for cat in p_item['categories']:
                categ = self.env['product.category'].sudo().search([('wooc_id', '=', cat['id'])], limit=1)
                if categ:
                    dict_p['categ_id'] = categ.id
                else:
                    dict_cat = {
                        'wooc_id': cat.get('id'),
                        'name': cat.get('name'),
                        'wooc_cat_slug': cat.get('slug'),
                        'wooc_cat_description': cat.get('description', ''),
                        'is_woocomm_category': True,
                        'is_exported': True,
                        'woocomm_instance_id': wooc_instance.id
                    }
        ...
        if p_item.get('attributes'):
            for attr in p_item['attributes']:
                ...
                p_attr_val = []
                if attr.get('options') and attr.get('variation'):
                    ...
                    for value in attr['options']:
                        ...
                        if existing_attr_value:
                            ...
                            new_attr_value = self.env['product.attribute.value'].sudo().create({
                                'name': value,
                                'attribute_id': product_attr.id,
                                'woocomm_attribute_id': attr['id']  # Include this line
                            })
                            p_attr_val.append(new_attr_value.id)

                    if product_attr and p_attr_val:
        ...
        if p_item.get('variations'):
            ...
            if product_variant:
                product_variant.sudo().write({
                    'woocomm_variant_id': p_item.get('id', ''),
                    'woocomm_instance_id': wooc_instance.id,
                    'is_exported': True
                })

            product.sudo().write({
                'list_price': float(p_item.get('price', 0.0)),
        ...
        for variant in self.get_all_products_variants(product.wooc_id, wooc_instance, limit=20):
            ...
            for v_item in product_variant:
                if v_item.product_template_attribute_value_ids:
                    list_values = [rec.name for rec in v_item.product_template_attribute_value_ids]
