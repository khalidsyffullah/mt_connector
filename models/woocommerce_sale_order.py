...
class SaleOrder(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            if vals['pricelist_id']:
                ...
                woocomm_instance_id = self.env['woocommerce.instance'].sudo().search(
                    [('id', '=', vals['woocomm_instance_id'])], limit=1)
                if pricelist_id.currency_id.name != woocomm_instance_id.wooc_currency:
                    raise UserError(
                        _("The Pricelist currency and WooCommerce currency does not match. \n\nPlease update the pricelist currency or authenticate WooCommerce instance again!!!"))
    ...
    def get_all_orders(self, wooc_instance, limit=100, max_retries=3):
        base_url = "https://lavishta.com/wp-json/wc/v3/orders"
        consumer_key = "ck_1fd87e77e4bb5864e3a716b9ebffa5ba7f75d6e2"
        consumer_secret = "cs_dc230f2aa6d3d61cbc21aa4e947adc4ff821fd8c"
        ...
        while get_next_page:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    url = f"{base_url}?orderby=id&order=asc&per_page={limit}&page={page}&consumer_key={consumer_key}&consumer_secret={consumer_secret}"
                    ...
                    if response.status_code == 200:
                        if response.content:
                            ...
                            if len(parsed_orders) > 0:
                                for order in parsed_orders:
                                    yield order
                                page += 1
                                retry_count = 0  # Reset retry count for the next page
                        ...
                        _logger.info(
                            '\n\n\n\n  Non-200 response for page= %s with status= %s and response= %s \n\n\n\n' % (
                                page, response.status_code, response.text))
                        retry_count += 1
                        time.sleep(2)  # Wait before retrying

                except Exception as error:
                    _logger.info('\n\n\n\n  Error Order on page= %s, retry= %s, error= %s \n\n\n\n' % (
                        page, retry_count, str(error)))
                    retry_count += 1
                    time.sleep(2)  # Wait before retrying

            if retry_count >= max_retries:
                _logger.info('\n\n\n\n  Max retries reached for page= %s \n\n\n\n' % (page))
                get_next_page = False
    ...
    def woo_temp_customer(self):
        while True:
            # Generate a random ID starting with 'tmp' followed by 5 digits
            temp_id = 'tmp' + ''.join(random.choices(string.digits, k=5))

            # Check if the ID already exists in the woo_temp_res_partner table
            self.env.cr.execute("""
                        SELECT 1 FROM woo_temp_res_partner WHERE wooc_temp_id = %s LIMIT 1
                    """, (temp_id,))
            existing_temp_id = self.env.cr.fetchone()
    ...
    def create_sale_order(self, order, instance_id):
        ...
        if not any(item.get('product_id') for item in order.get('line_items', [])):
            # Log the error into woo_order_log
            self.env.cr.execute("""
                  INSERT INTO woo_order_log (order_id, failed_reason,date_created)
                  VALUES (%s, %s, %s)
              """, (order['id'], 'no product exists for this order', order['date_created']))

            return  # Exit the method if no product_id is found

        if order['customer_id'] == 0 and order['status'] == 'cancelled':
            # Log the error into woo_order_log
            self.env.cr.execute("""
                  INSERT INTO woo_order_log (order_id, failed_reason,date_created)
                  VALUES (%s, %s, %s)
                  """, (order['id'], 'the order has a problem in customer data', order['date_created']))
            return  # Exit the method if customer_id is 0 and status is 'cancelled'
        ...
        if not res_partner:
            ...
            if not customer_data:
                # If customer_data is False, generate a temporary ID and prepare the data
                temp_id = self.woo_temp_customer()
                # Prepare customer_data
                customer_data = {
                    'id': temp_id,
                    'email': order.get('billing', {}).get('email', f"{temp_id}@temp.email.com"),
                    'first_name': order.get('billing', {}).get('first_name', "Temp"),
                    'last_name': order.get('billing', {}).get('last_name', "User"),
                    'role': "customer",
                    'username': f"{order.get('billing', {}).get('first_name', 'Temp')}.{order.get('billing', {}).get('last_name', 'User')}",
                    'billing': order.get('billing', {}),
                    'shipping': order.get('shipping', {})
                }
                # Insert temp_id and other details into woo_temp_res_partner table
                self.env.cr.execute("""
                    INSERT INTO woo_temp_res_partner (wooc_temp_id, name, email, phone_no)
                    VALUES (%s, %s, %s, %s)
                """, (
                    temp_id,
                    customer_data.get('first_name', ''),
                    customer_data.get('email', ''),
                    customer_data.get('billing', {}).get('phone', '')
                ))
                ...
                # If the customer already exists, proceed as usual
                customer_data = self.env['res.partner'].get_wooc_customer(order['customer_id'], instance_id)
                self.env['res.partner'].create_customer(customer_data, instance_id)
    ...
    def create_sale_order_line(self, so_id, instance_id, line_item):
        ...
        if line_item.get('product_id') or line_item.get('variation_id'):
            res_product = self.env['product.product'].sudo().search(
                ['|', ('woocomm_variant_id', '=', line_item.get('product_id')),
                 ('woocomm_variant_id', '=', line_item.get('variation_id'))],
                limit=1)
    ...
    def create_woocomm_shipping_lines(self, so_id, instance_id, order):
        for sh_line in order['shipping_lines']:
            shipping = self.env['delivery.carrier'].sudo().search(
                ['&', ('woocomm_method_id', '=', sh_line['method_id']), ('woocomm_instance_id', '=', instance_id.id)],
                limit=1)
            if not shipping:
    ...
    def create_woocomm_new_order(self, instance_id, order):
        ...
        for line in so_lines:
            ...
            if line.is_delivery:
                shipping_line = self.env['delivery.carrier'].sudo().search([('product_id', '=', line.product_id.id)],
                                                                           limit=1)
    ...
    def get_order_address(self, customer):
        ...
        contacts_billing = self.env['res.partner'].sudo().search(
            [('parent_id', '=', customer.id), ('type', '=', 'invoice')], limit=1)
        contacts_delivery = self.env['res.partner'].sudo().search(
            [('parent_id', '=', customer.id), ('type', '=', 'delivery')], limit=1)
