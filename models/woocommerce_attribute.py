...
class ProductAttribute(models.Model):
        ...
        if wooc_instance.is_authenticated:
            try:
                woo_api = API(
                            ...
                            timeout=300  # Add this line to set a 30-second timeout
    ...
    def create_attribute_terms(self, attr, wooc_instance):
        ...
        wc_attr_terms = woo_api.get(f"products/attributes/{attr_id}/terms")
            ...
            if wc_attr_terms.content:
                ...
                for value in attr_terms:
                    ...
                    dict_value = {
                        'wooc_id': value["id"],
                        'name': value["name"],
                        'wooc_slug': value["slug"],
                        'wooc_description': value["description"],
                        'is_woocomm': True,
                        'woocomm_instance_id': wooc_instance.id,
                        'attribute_id': attr.id,
                        'woocomm_attribute_id': attr.id,
                    }
    ...
    def wooc_attribute_terms_create(self, wooc_instance, p_attr, attr_wc_id, attr_val):
        _logger.info(f'wooc_attribute_terms_create: {attr_wc_id}')
        woo_api = self.init_wc_api(wooc_instance)
        ...
        if attr_val_exist.wooc_id:
            check_attr_term = woo_api.get(f"products/attributes/{attr_wc_id}/terms/{attr_val_exist.wooc_id}")
        ...
        if not attr_val_exist.wooc_id:
            ...
            wc_attr_term_data = woo_api.post(f"products/attributes/{attr_wc_id}/terms", data)
        ...
        if term_id:
            wc_attr_term = woo_api.get(f"products/attributes/{attr_wc_id}/terms/{term_id}").json()

        if wc_attr_term.get('id'):
            term_data = {
                "wooc_id": wc_attr_term['id'],
            ...
            if attr_val_exist:
                attr_val_exist.sudo().write(term_data)
            else:
                self.env['product.attribute.value'].sudo().create(term_data)
            self.env.cr.commit()
